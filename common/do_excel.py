...
class DoExcel:
    ...
    def get_cases(self, sheet_name):  # 根据sheet名称，获取在这个sheet里面的所有测试用例数据
        ...
        for r in range(2, max_row + 1):  # for 循环，从第二行开始遍历
            ...
            case.url = sheet.cell(row=r, column=2).value  # 取第r行，第4格的值
            case.data = sheet.cell(row=r, column=3).value  # 取第r行，第2格的值
            case.title = sheet.cell(row=r, column=4).value  # 取第r行，第3格的值
    ...
    def write_result_by_case_id(self, sheet_name, case_id, result):
        ...
        for r in range(2, max_row + 1):  # for 循环，从第二行开始遍历
            case_id_r = sheet.cell(r, 1).value  # 获取第r行，第1列，也就是case_id这一列的值
            if case_id_r == case_id:  # 判断excel里面取到的当前行的case id 是否等于传进来的case id
                sheet.cell(r, 8).value = result  # 写入传进来的actual到当前行的actual列的单元格
                self.workbook.save(filename=self.file_name)
                break

if __name__ == '__main__':
    do_excel = DoExcel(r'D:\1python12期\项目实战\api_test\datas\test_datas.xlsx')  # 实例化一个DoExcel对象
    sheet_names = do_excel.get_sheet_names()  # 获取到work boot里面所有的sheet名称的列表
    print("sheet 名称列表：", sheet_names)
    case_list = ['login', 'register','recharge']  # 定义一个执行测试用例的列表
    for sheet_name in sheet_names:
        if sheet_name in case_list:  # 如果当前的这个sheet_name 不在可执行的case_list里面，就不执行
            ...
            for case in cases:  # 遍历测试用例列表，每进for一次，就取一个case实例
                ...
                do_excel.write_actual_by_case_id(resp.get_text())  # 将相应的str类型的结果，写入到excel里面
